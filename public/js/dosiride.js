...
var same = hand.filter(function (elt){ return elt == card; }); 
var nb = $('#hand span.card' + card + ' i'); 
nb.text(same.length + 1); 
nb.show(); 
} 
hand.push(card); 
...
var index = $('#cards img').index(elt) 
 ...
 , img = $(elt);
  $.post("/jeu/"+dr.partie+"/" +dr.name+"/pioche/"+index, function ( data ) { 
  $( ".result" ).html( data ); 
